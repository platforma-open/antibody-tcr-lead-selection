...
# Expected input file has clonotypeKey, and one or two cdr3Sequence[.chain] columns,
# and one or two vGene[.chain] columns
...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        help=(
            "Input Parquet file with top/filtered clonotypes to calculate spectratype and V/J gene usage only on them."
        ),
    )
    parser.add_argument(
        "--spectratype_tsv", required=True, help="Output TSV file with chain, cdr3Length, vGene, and count columns."
    )
    parser.add_argument(
        "--vj_usage_tsv", required=True, help="Output TSV file with vGene, jGene, and count columns for V/J gene usage."
    )
    ...
    df_long = pd.wide_to_long(
        df, stubnames=["cdr3Sequence", "vGene", "jGene"], i="clonotypeKey", j="chain", sep=".", suffix=".+"
    ).reset_index()
    ...
    if df_long.empty:
        ...
        spectratype_df = (
            df_long.groupby(["chain", "cdr3Length", "vGene"])
            .size()
        ...
        vj_usage_df = df_long.groupby(["chain", "vGene", "jGene"]).size().reset_index(name="count").sort_values("count")
...
# Example usage:
# python software/spectratype/src/main.py --input_tsv cdr3_sequences_input.tsv \
#     --output_tsv 'cdr3_lengths.tsv' --vj_usage_tsv vj_usage.tsv
# python software/spectratype/src/main.py --input_tsv cdr3_sequences_input.tsv \
#     --output_tsv 'cdr3_lengths.tsv' --vj_usage_tsv vj_usage.tsv \
#     --final_clonotypes_csv topClonotypes.csv
